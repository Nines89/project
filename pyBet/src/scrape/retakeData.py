...
class PageResult:
    ...
    def find_stat(self):
        ...
        def Punizioni():
            regex = r"stat__homeValue\"\>(?P<pun1>\d+)\</div\>\<div class=\"stat__categoryName\">Punizioni\</div\>\<div class=\"stat__awayValue\"\>(?P<pun2>\d+)"
            matches = re.search(regex, self.txt, re.MULTILINE)
        ...
        def passComple():
            regex = r"stat__homeValue\"\>(?P<t1>\d+)\</div\>\<div class=\"stat__categoryName\">Passaggi \w+\</div\>\<div class=\"stat__awayValue\"\>(?P<t2>\d+)"
            matches = re.search(regex, self.txt, re.MULTILINE)
    ...
    def find_classifica(self):
        regex = r";\">(?P<pos1>\d+).[\s\S]+?tableCellParticipant__name\">(?P<team1>[\w\s]+)" \
                r"|title=\"\">(?P<pos2>\d+).[\s\S]+?tableCellParticipant__name\">(?P<team2>[\w\s]+)"
        matches = re.finditer(regex, self.txt, re.MULTILINE)
        for mat in matches:
    ...
    def find_forma(self):
        regex = r"title=\"\">(?P<pos>\d+).[\s\S]+?tableCellParticipant__name\">(?P<team>[\w\s]+)" \
    ...
    def n_Over_Under_05(self):
        regex = r"ui-table__row table__row--selected [\s\S]+?_name\">(?P<team>[\w\s]+)" \
...
def checkNormalMatch(txt):
    ...
    regex2 = r"<span class=\"fixedHeaderDuel__detailStatus\">A Tavolino</span>"
    matches = re.search(regex, txt, re.MULTILINE)
    mat = re.search(regex2, txt, re.MULTILINE)
    if matches:
        common = matches.group("days")
    if common != 'Giornata' or mat:
        return False
    else:
        return True

def last_regex(stringa):
    """
    Def of a function = def last_regex(stringa: Any) -> Tuple[List[int], List[int], List[str]]
    :param string: stringa contentenente gli ultimi cinque match
    :return: goal squadra 1, goal squadra 2, esito incontro
    """
    goal1 = []
    goal2 = []
    esito = []

    regex = r"\[\'[\w\s]+\', \'[\w\s]+\', \'(?P<goal1>\d+) : (?P<goal2>\d+)\', \'(?P<result>\w)"
    matches = re.finditer(regex, stringa, re.MULTILINE)
    for mat in matches:
        goal1.append(int(mat.group("goal1")))
        goal2.append(int(mat.group("goal2")))
        esito.append(mat.group("result"))

    return goal1, goal2, esito

def last_regex_vsv(stringa):
    """
    Def of a function = def last_regex(stringa: Any) -> Tuple[List[int], List[int]]
    :param stringa: stringa contentenente gli ultimi cinque match dei VS
    :return: goal squadra 1, goal squadra 2
    """
    goal1 = []
    goal2 = []
    regex = r"\[\'[\w\s]+\', \'[\w\s]+\', \'(?P<goal1>\d+) : (?P<goal2>\d+)\'"
    matches = re.finditer(regex, stringa, re.MULTILINE)
    for mat in matches:
        goal1.append(int(mat.group("goal1")))
        goal2.append(int(mat.group("goal2")))
